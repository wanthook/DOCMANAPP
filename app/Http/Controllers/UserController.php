...
class UserController extends Controller
{
    ...
    {
        $data   = $user->with('departemen')->where('hapus','1');
        ...
        return  Datatables::of($data)
                ...
                {
                    return $data->departemen->departemen_nama;
                })
                ->editColumn('id', 'ID: {{$id}}')
                ->make(true);
    }
    ...
    public function selectdua(Request $request, User $user)
    {
        ...
        if($request->input('id'))
        {
            $datas = $user->where('id',$request->input('id'));
        }
        else
        {
    ...
    public function selectduaauthor(Request $request, User $user)
    {
        $ret    = array();
        $datas  = array();
        if($request->input('id'))
        {
            $datas = $user->where('id',$request->input('id'))->where('type','ADMIN');
        }
        else
        {
            $datas = $user->where(function($query) use ($request)
                               {
                                    $query->where('name','like','%'.$request->input('q').'%')->where('type','ADMIN');
                               });
        }
        $datas = $datas->get()->toArray();
        ...
        foreach($datas as $data)
        {
            $ret[] = array('id' => $data['id'], 'text' => $data['name'].', '.$data['type']);
        }
